...
# ==== ROTEIRO 8 =====
    ...
    def arestas_com_custo_minimo_maximo(self):
        min, max = 99999999, 0
        for aresta in self.ListaArestas():
            peso = self.peso_aresta(aresta)
            if(peso > max):
                max = peso
            if(peso < min):
                min = peso
    ...
    """ Let us take another look at the search for the minimal edges. Denote by min
    ...
    def busca_aresta_com_menor_peso(self):
        menor_custo = 9999999999
        aresta = ''
        for aresta in arestas:
            custo_aresta = self.peso_aresta(aresta)
            if(custo_aresta < menor_custo):
                menor_custo = custo_aresta
                aresta = aresta
